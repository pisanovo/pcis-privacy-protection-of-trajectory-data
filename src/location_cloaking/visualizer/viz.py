...
# granularities
COLOR_GRANULE_L1 = pygame.Color(220, 220, 220)
COLOR_GRANULE_L2 = pygame.Color(211,211,211)
COLOR_GRANULE_L3 = pygame.Color(192,192,192)
COLOR_GRANULE_DEFAULT = pygame.Color(105,105,105)
...
class Util(object):
    ...
    def length(v):
        ...
        return math.sqrt(v.xodr_str ** 2 + v.y ** 2 + v.z ** 2)

    @staticmethod
    def get_bounding_box(actor):
        """Gets the bounding box corners of an actor in world space"""
        bb = actor.trigger_volume.extent
        corners = [carla.Location(x=-bb.xodr_str, y=-bb.y),
                   carla.Location(x=bb.xodr_str, y=-bb.y),
                   carla.Location(x=bb.xodr_str, y=bb.y),
                   carla.Location(x=-bb.xodr_str, y=bb.y),
                   carla.Location(x=-bb.xodr_str, y=-bb.y)]
...
class MapImage(object):
    ...
    def __init__(self, carla_world, carla_map, pixels_per_meter, show_triggers, show_connections, show_spawn_points):
        ...
        # print((self.min_x, self.min_y), (self.max_x, self.max_y), self.width)
    ...
    def draw_road_map(self, map_surface, carla_world, carla_map, world_to_pixel, world_to_pixel_width, dim, scale):
        """Draws all the roads, including lane markings, arrows and traffic signs"""
        map_surface.fill(COLOR_ALUMINIUM_4)
        precision = 0.05

        def _rec_render_test_granularities(level, max_level, min_x, max_x, min_y, max_y, surface, w, sc):
            line_border_size = int(math.ceil(4.0 * sc))
            line_color = COLOR_GRANULE_DEFAULT
            if level == 0:
                line_border_size = int(math.ceil(16.0 * sc))
                line_color = pygame.Color(119, 136, 153)
            elif level == 1:
                line_border_size = int(math.ceil(8.0 * sc))
                line_color = pygame.Color(47, 79, 79)
            elif level == 2:
                line_border_size = int(math.ceil(4.0 * sc))
                line_color = COLOR_GRANULE_L3

            width = max_x - min_x
            height = max_y - min_y

            if level < max_level:
                _rec_render_test_granularities(
                    level + 1,
                    max_level,
                    min_x,
                    min_x + (width / 2),
            ...
            corners_bl = [
                carla.Location(x=min_x, y=min_y),
                carla.Location(x=min_x + (width / 2), y=min_y),
                carla.Location(x=min_x + (width / 2), y=min_y + (height / 2)),
                carla.Location(x=min_x, y=min_y + (height / 2))
            ]
            corners_br = [
                carla.Location(x=min_x + (width / 2), y=min_y),
                carla.Location(x=max_x, y=min_y),
                carla.Location(x=max_x, y=min_y + (height / 2)),
                carla.Location(x=min_x + (width / 2), y=min_y + (height / 2))
            ]
            corners_tr = [
                carla.Location(x=min_x + (width / 2), y=min_y + (height / 2)),
                carla.Location(x=max_x, y=min_y + (height / 2)),
                carla.Location(x=max_x, y=max_y),
                carla.Location(x=min_x + (width / 2), y=max_y)
            ]
            corners_tl = [
                carla.Location(x=min_x, y=min_y + (height / 2)),
                carla.Location(x=min_x + (width / 2), y=min_y + (height / 2)),
                carla.Location(x=min_x + (width / 2), y=max_y),
                carla.Location(x=min_x, y=max_y)
            ]
            corners_bl = [world_to_pixel(p) for p in corners_bl]
            corners_br = [world_to_pixel(p) for p in corners_br]
            corners_tr = [world_to_pixel(p) for p in corners_tr]
            corners_tl = [world_to_pixel(p) for p in corners_tl]

            pygame.draw.lines(surface, line_color, True, corners_bl, line_border_size)
            pygame.draw.lines(surface, line_color, True, corners_br, line_border_size)
            pygame.draw.lines(surface, line_color, True, corners_tr, line_border_size)
            pygame.draw.lines(surface, line_color, True, corners_tl, line_border_size)

        def _render_test_granularities(surface, list_v, wtp, d, sc):
            max_level = 5
        ...
        def render_granularities(surface, wtp, d, sc):
            _render_test_granularities(surface, None, wtp, d, sc)
        ...
        def draw_topology(carla_topology, index):
            ...
            # Render granularities
            render_granularities(map_surface, world_to_pixel, dim, scale)
    ...
    def world_to_pixel(self, location, offset=(0, 0)):
        ...
        x = self.scale * self._pixels_per_meter * (location.xodr_str - self._world_offset[0])
...
class World(object):
    ...
    def __init__(self, name, args, data, timeout):
        self.client = None
        self.name = name
        self.args = args
        ...
        # Granularities
        # TODO: Turn this into an array of surfaces for each granularity (level)
        self.granularities_surface = None
        self.granules_surface = None
    ...
    def start(self, hud, input_control):
        ...
        # print(self.map_image.surface.get_width(), self.map_image.surface.get_height())

        # Render Granularities
        self.granularities_surface = pygame.Surface((self.map_image.surface.get_width(), self.map_image.surface.get_height()))
        self.granularities_surface.set_colorkey(COLOR_BLACK)
        self.granularities_surface.set_alpha(round(0.8 * 256))

        # Render Granules
        self.granules_surface = pygame.Surface((self.map_image.surface.get_width(), self.map_image.surface.get_height()))
        self.granules_surface.set_colorkey(COLOR_BLACK)
        self.granules_surface.set_alpha(0.25 * 256)
        ...
        pygame.draw.rect(self.border_round_surface, COLOR_WHITE, pygame.Rect(0, 0, self._hud.dim[0], self._hud.dim[1]))
        # pygame.draw.circle(self.border_round_surface, COLOR_WHITE, center_offset, int((self._hud.dim[1] - 8) / 2))

        scaled_original_size = self.original_surface_size * (1.0 / 0.9) * 2
    ...
    def update_hud_info(self, clock):
        ...
        if self.hero_actor is not None:
            ...
            hero_speed_text = 3.6 * math.sqrt(hero_speed.xodr_str ** 2 + hero_speed.y ** 2 + hero_speed.z ** 2)
            ...
            hero_mode_text = [
                ...
                f'Hero x: {self.hero_actor.get_location().xodr_str}',
                f'Hero y: {self.hero_actor.get_location().y}',
                'Hero Affected by:',
        ...
        for entry in self.granularity_data:
            data.append(f"{(entry.alias, entry.level)}")
    ...
    def _render_vehicles(self, surface, list_v, world_to_pixel):
        ...
        for v in list_v:
            ...
            corners = [carla.Location(x=-bb.xodr_str, y=-bb.y),
                       carla.Location(x=bb.xodr_str - 0.8, y=-bb.y),
                       carla.Location(x=bb.xodr_str, y=0),
                       carla.Location(x=bb.xodr_str - 0.8, y=bb.y),
                       carla.Location(x=-bb.xodr_str, y=bb.y),
                       carla.Location(x=-bb.xodr_str, y=-bb.y)
                       ]
    ...
    def _render_test_granules(self, surface, g_id_list, world_to_pixel, color):
        for granule_id in g_id_list:
            level = 0

            sum_granules_upto_level = lambda l: (4 / 3) * (math.pow(4, l) - 1)

            while granule_id >= sum_granules_upto_level(level + 1):
                level += 1

            num_granules_lower_levels = sum_granules_upto_level(level)

            map_width = self.map_image.max_x - self.map_image.min_x
            map_height = self.map_image.max_y - self.map_image.min_y
            level_granule_width = map_width / math.pow(2, level + 1)
            level_granule_height = map_height / math.pow(2, level + 1)
            granule_column_index = int((granule_id - num_granules_lower_levels) % math.pow(2, level + 1))
            # print(granule_id, level)
            # granule_row_index = math.floor(granule_id / math.pow(2, level + 1)) - level
            # print(accum, (granule_id - accum), math.pow(2, level + 1))
            granule_row_index = math.floor((granule_id - num_granules_lower_levels) / math.pow(2, level + 1))
            granule_min_x = self.map_image.min_x + (level_granule_width * granule_column_index)

            granule_min_y = self.map_image.min_y + (level_granule_height * granule_row_index)

            # print(granule_column_index, granule_row_index)
            # print()
            # print(self.map_image.max_x, self.map_image.min_x, level_granule_width, granule_min_x)
            # print(self.map_image.max_y, self.map_image.min_y, level_granule_height, granule_min_y)
            # print()

            corners_granules = [
                carla.Location(x=granule_min_x, y=granule_min_y),
                carla.Location(x=granule_min_x + level_granule_width, y=granule_min_y),
                carla.Location(x=granule_min_x + level_granule_width, y=granule_min_y + level_granule_height),
                carla.Location(x=granule_min_x, y=granule_min_y + level_granule_height)
            ]

            corners_granules = [world_to_pixel(p) for p in corners_granules]

            pygame.draw.polygon(
                surface,
                # pygame.Color(119, 136, 153),
                color,
                corners_granules,
                0
            )

    def render_granules(self, surface):
        g_id_list: List[VisualizeEntry] = self.granularity_data

        # list
        #   alias
        #   color
        #   granules
        for entry in g_id_list:
            if "CARLA-id-25" in entry.alias:
                # print(entry)
                # vicinity_shape = entry.vicinity_shape
                # if isinstance(vicinity_shape, VicinityCircleShape):
                self._render_test_granules(surface, entry.granularities[len(entry.granularities)-1].vicinity.granules, self.map_image.world_to_pixel, pygame.Color(255, 205, 105))
                # self._render_test_granules(surface, [entry.granularity.location.granule], self.map_image.world_to_pixel)
            else:
                self._render_test_granules(surface, entry.granularities[len(entry.granularities) - 1].vicinity.granules,
                                           self.map_image.world_to_pixel, pygame.Color(119, 136, 153))
                # self._render_test_granules(surface, [entry.granularity.location.granule], self.map_image.world_to_pixel)

    def render_vicinity_shapes(self, surface, vehicles):
        visualize_entries: List[VisualizeEntry] = self.granularity_data
        for vehicle in vehicles:
            vehicle = vehicle[0]
            matching_visualize_entries = [entry for entry in visualize_entries if f"CARLA-id-{vehicle.id}" in entry.alias]

            for match in matching_visualize_entries:
                vicinity_shape = match.vicinity_shape
                # print("BEFORE DRAW 1 ", vicinity_shape)
                v_location = vehicle.get_location()
                v_x_shift_location = carla.Location(x=v_location.xodr_str - vicinity_shape["radius"], y=v_location.y)
                location_x_shift = self.map_image.world_to_pixel(v_x_shift_location)
                location = self.map_image.world_to_pixel(vehicle.get_location())
                distance = math.sqrt((location[0] - location_x_shift[0])**2 + (location[1] - location_x_shift[1])**2)
                # print("BEFORE DRAW ", location)
                pygame.draw.circle(surface, (255, 255, 255), location, distance, int(math.ceil(4.0 * self.map_image.scale)))
    ...
    def clip_surfaces(self, clipping_rect):
        ...
        self.granularities_surface.set_clip(clipping_rect)
    ...
    def render(self, display):
        ...
        # if self.scaled_size != self.prev_scaled_size:
        self.granules_surface.fill(COLOR_BLACK)
        self.render_granules(self.granules_surface)

        self.actors_surface.fill(COLOR_BLACK)
        # Render vicinity shapes
        self.render_vicinity_shapes(self.actors_surface, vehicles)
        ...
        if self.hero_actor is not None:
            ...
            translation_offset = (hero_location_screen[0] - self.hero_surface.get_width() / 2 + hero_front.xodr_str * PIXELS_AHEAD_VEHICLE,
...
def game_loop(args, data):
    ...
    try:
        ...
        world = World(TITLE_WORLD, args, data, timeout=2.0)
        ...
        input_control.wheel_offset = 0.25
        input_control.mouse_offset = [0, 0]
        input_control.mouse_pos = [0, 0]
        input_control._world.scale_offset = [0, 0]
        input_control._world.hero_actor = None
        input_control._hud.notification('Map Mode')
...
def main(data=None):
    ...
    argparser = argparse.ArgumentParser(
        description='VIZ | Agent Vic and Pos granules')
